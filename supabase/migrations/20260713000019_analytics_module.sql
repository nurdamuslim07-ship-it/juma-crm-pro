-- Analytics module — see Analytics module requirements. Unlike
-- dashboard_summary() (SECURITY INVOKER, relying on each subquery's
-- own table RLS to naturally zero out unauthorized figures), this
-- module's role split is more specific than any existing table
-- permission expresses: "director: full analytics; manager: sales/
-- orders only; accountant: payment/finance only; every other role:
-- only their own KPI." Reusing e.g. `payments.read` (which manager
-- also holds, for the Payments module) or `orders.read_financial`
-- (which accountant also holds) would leak the wrong sections to the
-- wrong role for THIS module specifically. So, same as Employees/
-- Partners: dedicated `analytics.*` permissions + SECURITY DEFINER
-- RPCs that redact whole sections in Postgres based on them.
--
-- Read-only redaction here follows get_employees()'s convention, not
-- Employees' write-side convention: a caller without a section's
-- permission gets that section back as null / an empty set, not an
-- exception — viewing analytics you don't have a section for is an
-- expected, benign case (like a master only ever seeing their own
-- KPI row), not a blocked action worth raising over.
  ...
  return query
    select
      ...
      case when v_financial then (
        ...
        where ap.paid_at between p_start and p_end
      ) end,
...
-- Per-employee KPI — "Қызметкерлер бойынша KPI" — self-only unless the
-- caller holds `analytics.read_all_kpi` (director/manager); the
-- payments-recorded figures are additionally gated per-row by
-- `analytics.read_financial` OR being that row's own employee, so an
-- employee always sees their own recorded-payments total (it's their
-- own work, not a global financial figure) even without the broader
  ...
  return query
    select
      ...
      case when v_financial or e.user_id = auth.uid() then coalesce((
        ...
        where ap.recorded_by = e.user_id and ap.paid_at between p_start and p_end
      ), 0) end,
      case when v_financial or e.user_id = auth.uid() then coalesce((
        select sum(ap.amount_tiyn) from active_payments ap
        where ap.recorded_by = e.user_id and ap.paid_at between p_start and p_end
...
-- same read-redaction convention as the rest of this file.
